...
class EasyTierTomlCodecTest {
    ...
    fun buildPreservesEmptyNetworkSecret() {
        val toml = EasyTierTomlCodec.build(
    ...
    @Test
    fun defaultTomlHasLocalViewerHostname() {
        val toml = EasyTierTomlCodec.defaultToml()
        assertTrue(toml.contains("hostname = \"LocalViewer\""))
        assertTrue(toml.contains("instance_name = \"Default\""))
    }
}
